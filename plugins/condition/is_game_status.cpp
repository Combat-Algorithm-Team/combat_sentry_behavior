...
BT::NodeStatus IsGameStatusCondition::checkGameStart()
{
  ...
  auto msg = getInput<combat_rm_interfaces::msg::GameStatus>("key_port");
  if (!msg) {
    RCLCPP_ERROR(logger_, "game progress: %d", msg->game_progress);
...
{
  factory.registerNodeType<combat_sentry_behavior::IsGameStatusCondition>("IsGameStatus");
}
