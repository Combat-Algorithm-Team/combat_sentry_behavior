...
IsOutpostHpLowCondition::IsOutpostHpLowCondition(
  const std::string & name, const BT::NodeConfig & config)
: BT::SimpleConditionNode(name, std::bind(&IsOutpostHpLowCondition::checkOutpostHp, this), config)
{
}

BT::NodeStatus IsOutpostHpLowCondition::checkOutpostHp()
{
  ...
  return static_cast<int>(msg->ally_outpost_hp) < hp_threshold ? BT::NodeStatus::SUCCESS
                                                              : BT::NodeStatus::FAILURE;
}

BT::PortsList IsOutpostHpLowCondition::providedPorts()
{
  return {
    BT::InputPort<combat_rm_interfaces::msg::GameRobotHp>(
      "key_port", "{@referee_gameRobotHp}", "GameRobotHp port on blackboard"),
    BT::InputPort<int>(
      "hp_threshold", 500, "Return SUCCESS when ally_outpost_hp is lower than this value"),
  };
}
...
{
  factory.registerNodeType<combat_sentry_behavior::IsOutpostHpLowCondition>("IsOutpostHpLow");
}
